    ...
    if(m_map_matrix[mapX][mapY] == 1){
            ...
            if(testY%2 == 0) { //Top-left tile
                ...
                if(mapX > 0 && mapY < m_height)
                    if(m_map_matrix[mapX-1][mapY] == 0 ||
                       m_map_matrix[mapX-1][mapY] == 2 ||
                       m_map_matrix[mapX-1][mapY] == 4)
                       horizontal = false;

                if(mapX < m_width && mapY > 0)
                    if(m_map_matrix[mapX][mapY-1] == 0 ||
                       m_map_matrix[mapX][mapY-1] == 3 ||
                       m_map_matrix[mapX][mapY-1] == 4)
                        vertical = false;

                if(mapX > 0 && mapY > 0){
                    if(m_map_matrix[mapX-1][mapY-1] == 0||
                       m_map_matrix[mapX-1][mapY-1] == 2||
                       m_map_matrix[mapX-1][mapY-1] == 3||
                       m_map_matrix[mapX-1][mapY-1] == 4||
                       m_map_matrix[mapX][mapY-1] == 1)
                        diagonal = false;
                }
        ...
        // 05   0  00  05  55  
                ...
                if(mapX < m_width && mapY < m_height-1)
                    if(m_map_matrix[mapX][mapY+1] == 0 ||
                       m_map_matrix[mapX][mapY+1] == 1 ||
                       m_map_matrix[mapX][mapY+1] == 2)
                       vertical = false;

                if(mapX > 0 && mapY < m_height-1)
                    if(m_map_matrix[mapX-1][mapY+1] == 0 ||
                       m_map_matrix[mapX-1][mapY+1] == 1 ||
                       m_map_matrix[mapX-1][mapY+1] == 2 ||
                       m_map_matrix[mapX-1][mapY+1] == 4 ||
                       m_map_matrix[mapX-1][mapY] == 3)
                        diagonal = false;
...
void GameMap::draw(SDL_Surface *screen, int x_cam = 0, int y_cam = 0)
{
    ...
    for(int y=0; y<m_height*2; y++){
        for(int x=0; x < m_width*2; x++){
            if(m_map_borders[x][y] > 0)
            {
                ...
                bordersTarget -> x = x * m_tileSize/2 - x_cam;
                bordersTarget -> y = y * m_tileSize/2 - y_cam;
